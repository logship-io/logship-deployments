...
verbose_ship() {
    ...
    if [ "$opt_verbose" = 'true' ]; then
        printf "         ${bold}${white}*${normal}    _______                                                     \n"
        printf "           ${bold}${white}*${normal}_ |   ==|                                            _        \n"
        printf "           ||_|     |${blue}[||][||]${green}[||]${red}[||]${normal}[||]${black}[||][||]${green}[||]${cyan}[||]${normal}[||]${cyan}[||]${normal}|        \n"
        printf "          _||_|     |${cyan}[||]${magenta}[||]${green}[||][||]${black}[||]${red}[||]${normal}[||][||]${yellow}[||]${normal}[||]${magenta}[||]${normal}|___     \n"
        printf "         |......................................................o.../     \n"
        printf "         \........................................................./      \n"
        printf "${bold}${blue}_,_,~_,~')_,~')_,~')_,~')_,~')_,~')_~')_,~')_~')_,~')_,~')_,~')_,~')_,~')${normal}\n\n"
    fi
}
...
install_database() {
    ...
    verbose "Using temporary directory $tempdir"
    info "Downloading $service_name..."
    verbose "Download URL: https://ar.logship.io/release/${service_name}/${architecture}/${tag}"
    ensure wget "https://ar.logship.io/release/${service_name}/${architecture}/${tag}" -P "$tempdir" -q --show-progress
    verbose "Downloaded $service_name."
    run_or_sudo mkdir -p "$opt_data_root"
    run_or_sudo mkdir -p "$database_path"

    verbose "Extracting files..."
    ensure unzip -qq "$tempdir/$tag" -d "$tempdir/extract"
